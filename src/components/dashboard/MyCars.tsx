...
export default function MyCars({ cars, onRefresh }: MyCarsProps) {
  ...
  const t: Record<Lang, Record<string, string>> = {
    en: {
      ...
      addFirst: 'Add your first car to start earning.',
      listCar: 'List a car',
      available: 'Available',
      hidden: 'Hidden',
      view: 'View',
      ...
      failedDelete: 'Could not delete the listing. Please try again.',
      failedToggle: 'Could not update listing availability. Please try again.',
    },
    ru: {
      deleteConfirm: 'Вы уверены, что хотите удалить это объявление?',
      noListings: 'Пока нет объявлений',
      addFirst: 'Добавьте первый автомобиль и начните зарабатывать.',
      ...
      deleting: 'Удаление...',
      delete: 'Удалить',
      addAnother: 'Добавить ещё авто',
      failedDelete: 'Не удалось удалить объявление. Попробуйте снова.',
      failedToggle: 'Не удалось обновить доступность. Попробуйте снова.',
    },
    az: {
      deleteConfirm: 'Bu elanı silmək istədiyinizə əminsiniz?',
      noListings: 'Hələ elan yoxdur',
      addFirst: 'Qazanmağa başlamaq üçün ilk avtomobilinizi əlavə edin.',
      ...
      deleting: 'Silinir...',
      delete: 'Sil',
      addAnother: 'Daha bir avtomobil əlavə et',
      failedDelete: 'Elanı silmək mümkün olmadı. Yenidən cəhd edin.',
      failedToggle: 'Mövcudluğu yeniləmək mümkün olmadı. Yenidən cəhd edin.',
    },
  };
  const tx = t[lang];
  ...
  if (cars.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 className="font-semibold text-gray-900 mb-1">{tx.noListings}</h3>
        <p className="text-sm text-gray-500 mb-4">{tx.addFirst}</p>
        <a href="/list-car"
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge
                ...
                label={car.is_active ? tx.available : tx.hidden}
              />
            </div>
            <div className="flex items-center gap-2 mt-3">
              ...
              >
                {togglingId === car.id ? '...' : car.is_active ? tx.hide : tx.unhide}
              </button>
              <button
              ...
              >
                {deletingId === car.id ? tx.deleting : tx.delete}
              </button>
            </div>
          </div>
        </div>
