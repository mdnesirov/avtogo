    ...
    <div>
      ...
      <section className="bg-gray-900 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-20 md:py-28 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6">
            {tx.homeHeroLine1}<br />
            <span className="text-green-400">{tx.homeHeroLine2}</span>
          </h1>
          ...
          {/* Search bar — icon-only submit button so length never breaks layout */}
          <form action="/cars" method="GET"
            className="bg-white rounded-2xl p-2 flex items-center gap-2 max-w-2xl mx-auto shadow-xl"
          >
            <div className="flex items-center gap-2 flex-1 min-w-0 px-3">
              <MapPin size={18} className="text-gray-400 shrink-0" />
              <select name="location" className="w-full py-2.5 text-sm text-gray-900 bg-transparent border-none outline-none min-w-0">
            ...
            <div className="hidden sm:flex items-center gap-2 flex-1 min-w-0 px-3">
              <span className="text-xs text-gray-400 font-medium shrink-0">{tx.homeFrom}</span>
              <input type="date" name="startDate" className="w-full py-2.5 text-sm text-gray-900 bg-transparent border-none outline-none min-w-0" />
            </div>

            <div className="hidden sm:block w-px h-8 bg-gray-200 shrink-0" />

            <div className="hidden sm:flex items-center gap-2 flex-1 min-w-0 px-3">
              <span className="text-xs text-gray-400 font-medium shrink-0">{tx.homeTo}</span>
              <input type="date" name="endDate" className="w-full py-2.5 text-sm text-gray-900 bg-transparent border-none outline-none min-w-0" />
            </div>

            {/* Fixed-size icon-only button — never expands with translation text */}
            <button
              type="submit"
              aria-label={tx.browseCarsTitle}
              className="bg-green-600 hover:bg-green-700 text-white w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors"
            >
              <Search size={20} />
            </button>
          </form>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/cars" className="text-green-600 hover:text-green-700 text-sm font-medium whitespace-nowrap shrink-0">
            {tx.homeViewAll}
          </Link>
        </div>
      ...
      <section className="bg-green-600 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 text-center">
          ...
          <p className="text-green-100 text-lg max-w-xl mx-auto mb-8">{tx.homeIdleCarSubtitle}</p>
          <Link
            href="/list-car"
